...
import { newNameSession, checkoutNameSession, loginByNameSession, logoutByNameSession } from "../index";
...
app.get("/api/logout_by_name_session", logoutByNameSession(repo));
...
describe("logoutByNameSession", () => {
  ...
  it("changes session to logged out and returns a blank JSON response when a logged in session is given", async () => {
    const response = await request(app).get(
      "/api/logout_by_name_session?sessionID=logged_in_session"
    );
    expect(response.status).toBe(200);

    // check name session on redis
    const getAsync = promisify(rdc.get).bind(rdc);
    const ses = await getAsync("logged_in_session");
    const nameSession = JSON.parse(ses);
    expect(nameSession.isLoggedIn).toBe(false);
  });

  it("returns 403 when the given nameSession is already logged out", async () => {
    const response = await request(app).get(
      "/api/logout_by_name_session?sessionID=logged_out_session"
    );
    expect(response.status).toBe(403);
    expect(response.body.message).toBe("Name session already logged out. NameSession: logged_out_session");
  });

  it("returns 404 when the given nameSession doesn't exist on redis", async () => {
    const response = await request(app).get(
      "/api/logout_by_name_session?sessionID=nonexistent_session"
    );
    expect(response.status).toBe(404);
    expect(response.body.message).toBe("NameSession not found. NameSession: nonexistent_session");
  });
  ...
  it("returns 500 when sessionID is not given", async () => {
    const response = await request(app).get("/api/logout_by_name_session");
    expect(response.status).toBe(400);
    expect(response.body.message).toBe("sessionID is required");
  });
});
